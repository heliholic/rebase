...
Build and source-tree terms that agents often confuse:

**TARGET** — A unified MCU firmware target (`STM32F722`, `STM32H743`, …). Sources live in
`src/platform/<family>/target/<TARGET>/`. Bare `make` builds `STM32F722`.

**CONFIG** — A board overlay in the `src/config` submodule. It selects a TARGET and board
pins/features. Build with `make CONFIG=<board>`. Do not set `TARGET` and `CONFIG` together.
