...
Copy the template from `src/main/common/header.h` into every new Rotorflight file:
```c
/*
...
Any constants or enum values used in the PG structures are defined in the PG header files.
...
Any change in the external APIs **must** be documented in `Releases.md` under the
heading for the version being released (`# 5.0.0`, or `# 5.0.0-RC1` for a pre-release).
CI extracts that section for GitHub releases.
