...
static void showGpsPage(void)
{
    ...
    tfp_sprintf(lineBuffer, "RX: %u", (uint)dashboardGpsPacketCount);
    padHalfLineBuffer();
    i2c_OLED_set_line(dev, rowIndex);
    i2c_OLED_send_string(dev, lineBuffer);
    ...
    tfp_sprintf(lineBuffer, "Dt: %u", (uint)gpsSol.navIntervalMs);
    padHalfLineBuffer();
    i2c_OLED_set_line(dev, rowIndex);
    i2c_OLED_send_string(dev, lineBuffer);
...
static void showBatteryPage(void)
{
    ...
    if (batteryConfig()->currentMeterSource != CURRENT_METER_NONE) {
        ...
        // Amp: DDD.D mAh: DDDDD
        tfp_sprintf(lineBuffer, "Amp: %d.%d mAh: %u", (int)(amperage / 100), (int)((amperage % 100) / 10), (uint)getMAhDrawn());
        padLineBuffer();
        i2c_OLED_set_line(dev, rowIndex++);
        i2c_OLED_send_string(dev, lineBuffer);
