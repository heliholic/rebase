...
uint8_t crc8_update(uint8_t crc, const void *data, size_t length, uint8_t poly);
...
uint16_t crc16_update(uint16_t crc, const void *data, size_t length, uint16_t poly);
...
uint8_t crc8_xor_update(uint8_t crc, const void *data, size_t length);
void crc8_xor_sbuf_append(sbuf_t *dst, const void *data);

#define FNV_PRIME           16777619
#define FNV_OFFSET_BASIS    2166136261

uint32_t fnv_update(uint32_t hash, const void *data, size_t length);
