...
static inline float scaleRangefApply(scaleRangef_t *scale, float x)
{
...
static inline float slewLimit(float current, float target, float step)
{
    if (step > 0) {
...
static inline float slewUpLimit(float current, float target, float step)
{
    if (step > 0) {
        if (target > current + step)
            return current + step;
...
static inline float slewDownLimit(float current, float target, float step)
{
    if (step > 0) {
        if (target < current - step)
            return current - step;
...
static inline float slewUpDownLimit(float current, float target, float upstep, float downstep)
{
    if (upstep > 0 && target > current + upstep) {
        return current + upstep;
    }
    if (downstep > 0 && target < current - downstep) {
        return current - downstep;
    }
    return target;
}

static inline int applyDeadband(const int value, const int deadband)
{
    if (value > deadband)
